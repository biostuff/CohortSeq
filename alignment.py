...
@click.option('--samblasterLocation',default="samtools",help='Where is samblaster located')
...
def alignAndSort (mapper,sequences,sequencesReverse,bam,sampleNames,outputNames,reference,paired,markDuplicates,baq,gpu,ncores,maxMem,bamInMemory,samtoolsLocation,sambambaLocation,samblasterLocation,bwaLocation,ngmLocation,picardLocation,tempFolder,rg_pu,rg_pl,rg_id,rg_sm):
   ...
   for i in enumerate(sequences):
       ...
       if mapper == "NGM":   
           if paired == False:
               ngm("--rg-id",pat_rg_id,"--rg-sm",pat_rg_sm,"--rg-pu",rg_pu,"--rg-pl",rg_pl,"--rg-pg","NGM",t=ncores,r = reference,q = sequences[i],o = tempBam,b = True,g = gpu,paired = paired)
           else: 
               if markDuplicates == "samblaster":
                   ...
                   p1 = subprocess.Popen([ngmLocation,"--rg-id",str(sequences[i]),"--rg-sm",str(sequences[i]),"--rg-pu",rg_pu,"--rg-pl",rg_pl,"--rg-pg","NGM","-1",sequences[i],"-2",sequencesReverse[i],"-r",reference,"-t",str(mapcores),gpu_string,"--paired"],stdout=subprocess.PIPE)
                   ...
                   ngm("--rg-id",pat_rg_id,"--rg-sm",pat_rg_sm,"--rg-pu",rg_pu,"--rg-pl",rg_pl,"--rg-pg","NGM","-1",sequences[i],"-2",sequencesReverse[i],r = reference,t=ncores,o = tempBam,b = True,strata = False,g = gpu,paired = paired,_no_out=False)    
       ...
       if mapper == "BWA": 
           ...
           if paired == False:
               if bam == True:
                   p1 = subprocess.Popen([samtoolsLocation,"bam2fq",sequences[i]],stdout=subprocess.PIPE)  
                   ...
                   p2 = subprocess.Popen([bwaLocation,"mem",reference,sequences[i],"-t",str(mapcores),"-R",R,"-"],stdout=subprocess.PIPE)
           else:
               if markDuplicates == "samblaster":
                   p2bwa = subprocess.Popen([bwaLocation,"mem",reference,sequences[i],sequencesReverse[i],"-t",str(mapcores),"-R",R,"-"],stdout=subprocess.PIPE)
                   p2 = subprocess.Popen([samblasterLocation],stdin=p2bwa.stdout,stdout=subprocess.PIPE)
                   p2bwa.stdout.close()
               else:
                   p2 = subprocess.Popen([bwaLocation,"mem",reference,sequences[i],sequencesReverse[i],"-t",str(mapcores),"-R",R,"-"],stdout=subprocess.PIPE)
           p3 = subprocess.Popen([samtoolsLocation,"view","-S","-b","-1","-@",str(mapcores),"-"], stdin=p2.stdout,stdout=outputter)
           p2.stdout.close()
       ...
       if baq == True:
           if markDuplicates == "sammbamba":
               outputter = open(tempBam,mode = "r+") 
           else:
               outputter = open(outputNames[i],mode = "w")
           ...
           p2 = subprocess.Popen([samtoolsLocation,"fillmd","-Erb","-",reference], stdin=p1.stdout,stdout=outputter) #send p1's output to p2
           p1.stdout.close() #make sure we close the output so p2 doesn't hang waiting for more input
           p2.communicate()[0] #run our command
           outputter.close()  
           if markDuplicates == "sambamba":
               sambamba.index(outputNames[i],t = str(ncores))
         
       else:
           if markDuplicates == "sambamba": 
               samtools.sort("-o",tempBam,"-O","bam","-@",str(ncores),"-m",str(int(math.floor(maxMem/ncores)))+"G","-T",directoryTemp + "/" + "sorttempbam/",tempBam)                  
           else:
               sambamba.sort(tempBam,o = outputNames[i],t = str(ncores),tmpdir = directoryTemp + "/" + "sorttempbam",m = str(maxMem) + "GB") 
        
       
       #mark  the duplicates
       if markDuplicates == True:
           sambamba.markdup(tempBam,outputNames[i],t = ncores,tmpdir = directoryTemp,r = False)
